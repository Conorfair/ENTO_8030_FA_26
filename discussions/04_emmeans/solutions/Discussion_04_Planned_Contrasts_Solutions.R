...
#
# Example answer (verify the cell order printed by emm_cells):
# H0 in words:
# The selected treatment-cell means do not differ according to the
# prespecified biological comparison.
#
# H1 in words:
# The selected treatment-cell means differ.
#
# Contrast weights:
# The exact weights depend on the hypothesis and the order printed by
# emm_cells. A valid contrast must sum to zero.
#
# Example check:
sum(c(-1, 1/3, 1/3, 1/3))
...
# ============================================================
# INSTRUCTOR NOTES / SUGGESTED DISCUSSION POINTS
# ============================================================

# ACTIVE LEARNING 1
# The interaction asks whether the effect of fertilizer depends on light
# treatment (equivalently, whether the effect of light depends on fertilizer).
# When an interaction is substantively important, conditional/simple effects
# are usually more informative than isolated marginal main effects.

# ACTIVE LEARNING 2
# Students should identify the conditioning factor explicitly:
# fert | light = fertilizer comparisons separately at each light level.
# light | fert = light comparisons separately at each fertilizer level.
# Emphasize estimate + CI + adjusted p-value + biological interpretation.

# ACTIVE LEARNING 3
# A valid contrast has coefficients summing to zero.
# Orthogonality is a separate property: two contrasts are orthogonal when
# their weighted cross-products equal zero (with additional weighting needed
# in unbalanced designs). Do not teach "sums to zero = orthogonal."

# ACTIVE LEARNING 4
# Pairwise comparisons increase rapidly:
# choose(k, 2).
choose(3, 2)
choose(5, 2)
choose(10, 2)
...
# ACTIVE LEARNING 5
# Dunnett is preferable if the prespecified scientific goal is to compare
# every treatment with one control. Tukey is appropriate when all pairwise
# differences are scientifically relevant.

# MANUSCRIPT EMPHASIS
# Students should avoid treating CLD letters as the scientific result.
# Prefer reporting model-based estimates, uncertainty, and the biological
# question addressed by the contrast.
