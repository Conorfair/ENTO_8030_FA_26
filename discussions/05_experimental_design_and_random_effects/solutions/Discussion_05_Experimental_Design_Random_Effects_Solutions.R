...
#
# ACTIVE LEARNING 1 ------------------------------------------
# 1. Experimental unit: the field plot, because insecticide is assigned to plots.
#
# 2. Observational/sampling unit: the individual pitfall trap.
#
# 3. There are 24 independent treatment replicates total (6 plots per treatment
#    if allocation is balanced). There are 120 trap observations, but n is not 120
#    for the insecticide treatment effect.
#
# 4. No. More traps increase subsampling within plots, not independent treatment
#    replication.
#
# Key rule:
...
#
# 1. Yes, it is still a valid CRD randomization, but a strong known row gradient
#    may reduce precision if it is ignored by the design.
#
# 2. Yes. Group similar experimental units along the gradient into blocks and
#    randomize treatments within blocks.
#
# 3. A randomized complete block design (RCBD), provided every treatment can be
...
# ACTIVE LEARNING 3 ------------------------------------------
# 1. The individual plot/position within a block is the experimental unit because
#    treatment is independently assigned at that level.
#
# 2. Six independent replicates per treatment, one in each block.
#
# 3. Block is a nuisance/grouping source of variation rather than the treatment
#    effect of primary biological interest.
#
# 4. Randomization within blocks prevents treatment from being systematically
#    associated with position inside a block while preserving the blocking structure.
...
#
# 1. Yes. The connected observations should show block-to-block shifts in baseline
#    biomass.
#
# 2. Units within a block share environmental conditions represented by the same
#    block-level deviation.
#
# 3. Ignoring block discards a known source of structure/variation and can give
#    inappropriate uncertainty for treatment effects.
...
#
# 1. The treatment point estimates should be broadly similar because the balanced
#    RCBD separates treatment allocation from block.
#
# 2. Yes. The mixed model partitions block-to-block variation separately from
#    within-block residual variation, changing the uncertainty used for inference.
#
# 3. It estimates how much baseline biomass varies among blocks after accounting
#    for treatment.
#
# 4. Each block receives its own deviation from the population intercept, while
#    those deviations are modeled as arising from a common distribution.
...
# ACTIVE LEARNING 6 ------------------------------------------
# 1. They are model-based treatment means estimated from the mixed model while
#    accounting for the block variance represented by the model.
#
# 2. The mixed-model comparisons use the uncertainty implied by the experimental
#    design rather than pretending all observations lack block structure.
#
# 3. Example: "Estimated insect biomass was higher under the High treatment than
#    the Control treatment (estimated difference = [use output] mg, 95% CI =
#    [use output], Tukey-adjusted p = [use output])."
...
#
# 1. Irrigation experimental unit: the whole plot.
#
# 2. Insecticide experimental unit: the subplot within a whole plot.
#
# 3. There are 12 independently randomized whole plots (2 within each of 6 blocks).
#    Each irrigation level occurs once per block, so each irrigation treatment has
#    6 independently randomized whole-plot replicates.
#
# 4. There are 36 subplot observations.
#
# 5. The three subplots within a whole plot all received the same irrigation
#    assignment. They are not three independent randomizations of irrigation.
...
# ACTIVE LEARNING 8 ------------------------------------------
# 1. irrigation, insecticide, and irrigation × insecticide are fixed treatment
#    effects.
#
# 2. block and whole_plot_id represent the block and whole-plot levels of the
#    randomization structure.
#
# 3. Irrigation was randomized to whole plots, so its true replication and
#    uncertainty are tied to whole-plot-to-whole-plot variation rather than to all
#    36 subplot observations as if they were independent irrigation assignments.
#
# 4. An interaction means the insecticide effect may depend on irrigation level;
#    conditional EMMs directly describe those treatment comparisons.
