...
## reading it relative to the Working Directory
...
#
#
# The pipe: "knits" multiple functions together 
#
# %>%
#
...
## create the object "garden_summary" from the object "gardens" and 
## group (categorize) by garden,
## sumarize= summary statistics of choice
...
## does the "ozone" vary between the "garden" data
...
#
## Based on the p-value and confidence interval, we have enough evidence to 
## reject the null hypothesis and determine that the difference between Garden 
## A and Garden B is statistically significant
## (mean difference: 2 (0.915-3.085), p-value= 0.0011)

# CGF Edit:
# This is a great sentence describing the statistical evidence to discuss the results. You have included the effect size, confidence interval and p-value. The t-test statistic and degrees of freedom could be included for completeness. I would also include the unit of measurement when describing the mean difference.
...
# var.equal = TRUE
## default setting is false (does not assume equal variance)
#
...
## NOTE: a Bartlett.test() could test whether or not the data has equal variance
...
## difference between the two plots is position geom_col(position = "fill") vs 
...
## No evidence that suggests that it does
#
# How is this scientific question different from the ladybird
# test of independence?
## We have one categorical variable across levels, and not testing associations
...
#
## Test: two-sample t-test (Welch's t-test, assumes unequal variance)
## can test for equal variance using the Bartlett or Levene test
#
#
# SCENARIO B
#
...
garden_median_plot <- ggplot(
  ...
  labs(
    title = "Comparison of the Medians of Ozone in Garden A and Garden B",
    x = "Garden",
    y = "Median Ozone (ppb)",
    fill = "Color morph"
  ) +
...
# CGF Edits: You had the explanation of the results above that go over the t-test on these data. You also create a figure that shows the different gardens, but the color morph code was copied from the other figure made above - a small oversight. 

# We will also talk about instances where bar graphs like this are appropriate over box-and-whisker plots used in the mean data above.
...
## All cells have the same departure from the expected values. 
#
#
## Black ladybirds are over represented in the Industrial habitat and 
## underrepresented in the Rural habitat, while the red ladybirds are under-
## represented in the Industrial habitat and over represented in the Rural
## habitat. 

# CGF_Edits: This sentence above does a good job of including the biologically relevant information about the results, but the statistical results are missing. We will see examples of manuscript langauge for the different sections (i.e, results vs. discussion).
